"""Prometheus metrics and request correlation middleware.

Provides:
- Request ID middleware: Generates/propagates correlation IDs for tracing
- Prometheus metrics: Request counts, latencies, active users, container counts
...
class MetricsRegistry:
    ...
    Uses simple counters and histograms suitable for single-process deployments.
    For multi-replica production deployments, replace with the `prometheus_client`
    library and push to a Pushgateway or use a metrics aggregation layer.
    """
    ...
    def generate(self) -> str:
        ...
        for key, value in sorted(self._counters.items()):
            if "{" in key:
                lines.append(f"carbon_agent_request_total {value}  # {key}")
            else:
                lines.append(f"carbon_agent_{key}_total {value}")

        # Gauges
        for key, value in sorted(self._gauges.items()):
            if "{" in key:
                lines.append(f"carbon_agent_gauge {value}  # {key}")
            else:
                lines.append(f"carbon_agent_{key} {value}")
        ...
        for key, values in sorted(self._histograms.items()):
            ...
            lines.append(f"carbon_agent_{name}_count {len(values)}")
            lines.append(f"carbon_agent_{name}_avg_seconds {avg:.4f}")
            lines.append(f"carbon_agent_{name}_max_seconds {max(values):.4f}")
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    ...
    - Checks for X-Request-ID header from upstream (e.g., Traefik)
