...
scoped_req_pattern = (
    r"(\s+)scoped_request = build_scoped_rag_request\(payload, principal\)"
)
...
where_filter_body_pattern = (
    r'(\s+)scoped_filter = build_scoped_rag_request\(\{\}, principal\)\["scope"\]'
)
...
where_filter_call_pattern = (
    r"(\s*\"where_filter\": _build_scoped_where_filter\()principal(,\s*\{[^}]+\})(\))"
)
...
where_filter_call_stats_pattern = (
    r"(\s*\"where_filter\": _build_scoped_where_filter\()principal(\))"
)
...
content = re.sub(
    where_filter_call_stats_pattern, replace_where_filter_call_stats, content
)
