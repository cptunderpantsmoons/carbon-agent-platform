...
for i, line in enumerate(lines):
    if '\"where_filter\": _build_scoped_where_filter(\"where_filter\": _build_scoped_where_filter' in line:
        # Replace with correct line
        lines[i] = '            \"where_filter\": _build_scoped_where_filter(principal, None, tenant_id),\n'
        break
...
for i, line in enumerate(lines):
    ...
    if in_delete and line.strip() == '_ensure_active_principal(principal)':
        ...
        while j < len(lines) and lines[j].strip() == '':
            j += 1
        if j < len(lines) and 'tenant_id = _get_tenant_id_from_request(request)' not in lines[j]:
        ...
        for k in range(j, len(lines)):
            if '\"where_filter\": _build_scoped_where_filter' in lines[k]:
                ...
                if 'tenant_id' not in lines[k]:
                    lines[k] = lines[k].replace('_build_scoped_where_filter(\n                principal,\n                {\"document_id\": document_id},\n            )', '_build_scoped_where_filter(\n                principal,\n                {\"document_id\": document_id},\n                tenant_id,\n            )')
                break
        break
